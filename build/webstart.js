    ...
    switch (stmt.tag) {
        ...
        case "if":
            ...
            for (var i = 0; i < stmt.ifs.length; i++) {
                if (i == 0) {
                    var cond = codeGenExpr(stmt.ifs[0].condition, locals).flat();
                    var condCode = cond.join("\n");
                    var ifbody = stmt.ifs[0].body.map(function (s) { return codeGenStmt(s, withParamsAndVariables_1); }).flat();
                    var bodyCode = ifbody.join("\n");
                    ifCode = ("\n          " + condCode + "\n          (if\n\n            (then\n\n              " + bodyCode + "\n            )\n          ");
                }
                else {
                    var condCode = codeGenExpr(stmt.ifs[0].condition, locals);
                    var ifbody = stmt.ifs[0].body.map(function (s) { return codeGenStmt(s, withParamsAndVariables_1); }).flat();
                    var bodyCode = ifbody.join("\n");
                    var exifCode = ("\n          (else\n          " + condCode + "\n          (if\n          (then\n            " + bodyCode + "\n          )\n          ");
